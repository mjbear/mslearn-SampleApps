    ...
    class QuarterlyIncomeReport
    {
        ...
        public SalesData[] GenerateSalesData()
        {
            ...
            for (int i = 0; i < 100000; i++)
            {
                try
                {
                    ...
                    ValidateSalesData(salesData[i]);
                }
                catch (ArgumentException ex)
                {
                    Console.WriteLine($"Error generating sales data for record {i}: {ex.Message}");
                    // Handle specific argument-related errors if necessary
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"An unexpected error occurred while generating sales data for record {i}: {ex.Message}");
        ...
        private void ValidateSalesData(SalesData data)
        {
            try
            {
                ...
                if (data.baseCost <= 0 || data.baseCost >= data.unitPrice)
                {
                    throw new ArgumentOutOfRangeException(nameof(data.baseCost), "Base cost must be greater than zero and less than unit price.");
                }
        
                if (data.quantitySold <= 0)
                {
                    throw new ArgumentOutOfRangeException(nameof(data.quantitySold), "Quantity sold must be greater than zero.");
                }
        
                if (string.IsNullOrEmpty(data.productID))
                {
                    throw new ArgumentNullException(nameof(data.productID), "Product ID cannot be null or empty.");
                }
        
                if (string.IsNullOrEmpty(data.departmentName))
                {
                    throw new ArgumentNullException(nameof(data.departmentName), "Department name cannot be null or empty.");
                }
            }
        ...
        public static string ConstructProductId(SalesData salesDataItem, Random random)
        {
            try
            {
                ...
                if (indexOfDept == -1)
                {
                    throw new ArgumentException($"Invalid department name: {salesDataItem.departmentName}");
                }
        ...
        public static string[,] DeconstructProductId(string productID)
        {
            try
            {
                ...
                if (parts.Length != 5)
                {
                    throw new ArgumentException("Product ID must contain exactly 5 parts separated by hyphens.");
                }
        ...
        public void QuarterlySalesReport(SalesData[] salesData)
        {
            ...
            TopThree(top3SalesOrdersByQuarter);
        
            // display the quarterly sales report
            bool reportsDisplayed = DisplayReports(quarterlySales, quarterlyProfit, quarterlyProfitPercentage, quarterlySalesByDepartment, quarterlyProfitByDepartment, quarterlyProfitPercentageByDepartment, top3SalesOrdersByQuarter, quarterlyTopProfitForProductNumbers);

            if (!reportsDisplayed)
            {
                throw new Exception("Failed to display the quarterly sales report.");
            }
        }
        ...
        private void TopThree(Dictionary<string, List<SalesData>> top3SalesOrdersByQuarter)
        {
            ...
            Dictionary<string, Dictionary<string, (int, double, double, double, double)>> quarterlyTopProfitForProductNumbers)
        {
            ...
            {
                Console.WriteLine($"An error occurred while displaying the reports: {ex.Message}");
                // Optionally log the error
                // Logger.LogError(ex);
